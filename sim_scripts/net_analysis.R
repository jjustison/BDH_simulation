...
col_names<-c('rets','tips','level','tree_child','tree_based','fu_stable','normal','gen_no','degen_no','neu_no','rec_rets','rec_tips','rec_level','rec_tree_child','rec_tree_based','rec_fu_stable','rec_normal','rec_gen_no','rec_degen_no','rec_neu_no')
...
my_dat$normal[!my_dat$tree_based] <- F ##Anything that is NOT tree child will NOT be normal
my_dat$normal[is.na(my_dat$normal)] <- unlist(lapply(phys[is.na(my_dat$normal)],isNormal)) ##Check the remaining Networks


###Obtain the Reconstructed network and record the same summary stats

phys<-lapply(phys,reconstructedNetwork)

my_dat$rec_rets<-(unlist(lapply(phys,function(x) nrow(x$reticulation))))
my_dat$rec_tips<-unlist(lapply(phys,function(x) length(x$tip.label)))

##Record the types of hybridization that are observed
my_dat[,c('rec_gen_no','rec_degen_no','rec_neu_no')] <- do.call(rbind,lapply(phys,get_hyb_nos))

##Record classes of networks
my_dat$rec_level<-unlist(lapply(phys,getNetworkLevel))
my_dat$rec_tree_child<-unlist(lapply(phys,isTreeChild))

my_dat$rec_tree_based<-my_dat$rec_tree_child ##Anything that is tree child is also tree based
my_dat$rec_tree_based[!my_dat$rec_tree_based]<-unlist(lapply(phys[!my_dat$rec_tree_based],isTreeBased)) ##Check the remaining networks to see if they're tree based
my_dat$rec_fu_stable[my_dat$rec_tree_child]  <- T ##Anything that is tree child is also fu stable
my_dat$rec_fu_stable[!my_dat$rec_tree_based] <- F ##Anything that is NOT tree based will NOT be fu stable
my_dat$rec_fu_stable[is.na(my_dat$rec_fu_stable)] <- unlist(lapply(phys[is.na(my_dat$rec_fu_stable)],isFUstable)) ##Check the remaining Networks
my_dat$rec_normal[!my_dat$rec_normal] <- F ##Anything that is NOT tree child will NOT be normal
my_dat$rec_normal[is.na(my_dat$rec_normal)] <- unlist(lapply(phys[is.na(my_dat$rec_normal)],isNormal)) ##Check the remaining Networks


#my_dat$rec_tree_based<-unlist(lapply(phys,isTreeBased))
#my_dat$rec_fu_stable <-unlist(lapply(phys,isFUstable))
